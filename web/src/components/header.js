...
export default function Header({ suffix }) {
  ...
  const btnDisabled =
    btnState === BTN_STATES.disabled || btnState === BTN_STATES.inflight;
  const btnAnimate = btnState === BTN_STATES.inflight;
    ...
    <header className="flex align-item justify-center flex-col items-center">
      ...
      <div className="max-w-xl">
        <form className="flex mt-10" onSubmit={onSubmit}>
          ...
          <button
            type="submit"
            className="flex align-items items-center bg-blue-600 text-white rounded-md ml-2 px-8 hover:bg-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed"
          ...
          >
            ...
            {btnAnimate && (
              <svg
                aria-hidden="true"
                class={`w-4 h-4 ml-1 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600`}
                viewBox="0 0 100 101"
                fill="none"
